...
function es(){let x;try{x=JSON.parse(localStorage.getItem('waterTracker_engagement_v1')||'{}')}catch{x={}};x.u=x.u||x.unlocked||{};return x}
function ss(x){localStorage.setItem('waterTracker_engagement_v1',JSON.stringify({u:x.u||{}}))}
...
function hourOf(d){let dt=new Date(d.at||Date.now());return isNaN(dt.getTime())?99:dt.getHours()}
function st(){let oz=0,drinks=0,goals=0,early=0,manual=0;Object.keys(state.days||{}).forEach(k=>{let ds=state.days[k].drinks||[];let t=ds.reduce((a,d)=>a+Number(d.oz||0),0);oz+=t;drinks+=ds.length;if(t>=goalFor(k))goals++;ds.forEach(d=>{if(hourOf(d)<9)early++;if(d.label==='Manual edit')manual++;});});let r=streaks();return{oz,drinks,goals,early,manual,best:r.best};}
