(function () {
  ...
  function calculate(api) {
    ...
    keys.forEach((key) => {
      ...
      if (goal > 0 && total >= goal) goalDays++;
      const sorted = [...drinks].sort(
        (a, b) => new Date(a.at) - new Date(b.at),
      );
      sorted.forEach((d) => {
        labels[d.label || "Drink"] = (labels[d.label || "Drink"] || 0) + 1;
        const dt = new Date(d.at);
        if (!Number.isNaN(dt.getTime()))
          hours[dt.getHours()] = (hours[dt.getHours()] || 0) + 1;
      });
      if (sorted[0]) {
      ...
      for (const drink of sorted) {
        ...
        if (goal > 0 && running >= goal) {
          const reached = new Date(drink.at);
          if (!Number.isNaN(reached.getTime()))
            goalTimes.push(reached.getHours() * 60 + reached.getMinutes());
          break;
        }
      }
    });
    const met = (key) => {
      ...
      return goal > 0 && api.totalFor(key) >= goal;
    };
    let bestStreak = 0,
      run = 0;
    if (keys.length) {
      let cursor = D.dateFromKey(keys[0]),
        end = D.dateFromKey(today);
      while (cursor <= end) {
        const key = D.dayKey(cursor);
        run = met(key) ? run + 1 : 0;
    ...
    let currentStreak = 0,
      cursor = D.dateFromKey(today);
    if (!met(today)) cursor.setDate(cursor.getDate() - 1);
    const earliest = keys.length ? D.dateFromKey(keys[0]) : null;
    while (earliest && cursor >= earliest && met(D.dayKey(cursor))) {
      currentStreak++;
      cursor.setDate(cursor.getDate() - 1);
    }
    const daysTracked = keys.filter((k) => api.drinksFor(k).length).length,
      avgDaily = daysTracked ? lifetimeOz / daysTracked : 0,
      completionRate = daysTracked ? (goalDays / daysTracked) * 100 : 0,
      ...
      top = (o) => Object.entries(o).sort((a, b) => b[1] - a[1])[0]?.[0] || "";
    const commonHour = top(hours);
    return {
      lifetimeOz,
      ...
      favoriteLabel: top(labels),
      mostCommonHour:
        commonHour === ""
          ? ""
          : new Date(2000, 0, 1, Number(commonHour)).toLocaleTimeString([], {
              hour: "numeric",
            }),
