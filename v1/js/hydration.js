  ...
  function createApi(initialState, onChange) {
    ...
    function commit(syncExternal = true) {
      ...
      window.dispatchEvent(
        new CustomEvent("wt-data-changed", { detail: { source: "hydration" } }),
      );
      return state;
    }
    function getState() {
    ...
    function isFutureDay(key) {
      return String(key) > D.dayKey();
    }
    function assertEditableDay(key) {
      if (
        !/^\d{4}-\d{2}-\d{2}$/.test(String(key)) ||
        D.dayKey(D.dateFromKey(key)) !== key
      )
        throw new Error("Choose a valid day.");
      if (isFutureDay(key)) throw new Error("Future days cannot be edited.");
    }
    ...
    function drinksFor(key = D.dayKey()) {
      return state.days[key]?.drinks || [];
    }
    function totalFor(key = D.dayKey()) {
      return drinksFor(key).reduce((sum, d) => sum + Number(d.oz || 0), 0);
    }
    function backupDay(key = D.dayKey()) {
      assertEditableDay(key);
      state.backups[key] = JSON.parse(JSON.stringify(ensureDay(key)));
      Object.keys(state.backups)
        .sort()
        .slice(0, -30)
        .forEach((k) => delete state.backups[k]);
    }
    function addDrink(oz, label = "Quick add", key = D.dayKey(), at = null) {
      assertEditableDay(key);
      const amount = Number(oz);
      if (!Number.isFinite(amount) || amount < 1 || amount > C.maxDrinkOz)
        throw new Error(`Enter an amount between 1 and ${C.maxDrinkOz} oz.`);
      backupDay(key);
      ensureDay(key).drinks.push({
        id: uid(),
        oz: amount,
        label: String(label || "Drink").slice(0, 80),
        at: at || D.isoForDay(key),
      });
      commit();
    ...
    function resetDay(key = D.dayKey()) {
      assertEditableDay(key);
      backupDay(key);
    ...
    function restoreDay(key = D.dayKey()) {
      ...
      state.days[key] = JSON.parse(JSON.stringify(state.backups[key]));
      commit();
      return true;
    }
    function saveCup(cup) {
      const next = {
        id: cup.id || uid(),
        name: String(cup.name || "").trim(),
        oz: Number(cup.oz),
      };
      if (
        !next.name ||
        !Number.isFinite(next.oz) ||
        next.oz < 1 ||
        next.oz > C.maxCupOz
      )
        throw new Error(
          `Enter a cup name and an amount between 1 and ${C.maxCupOz} oz.`,
        );
      next.name = next.name.slice(0, 40);
      const i = state.cups.findIndex((c) => c.id === next.id);
      if (i >= 0) state.cups[i] = next;
      else state.cups.push(next);
      commit();
      return next;
    }
    ...
    function saveSettings(settings) {
      state.settings = S.validateSettingsInput(settings, state.settings);
      commit();
    }
    ...
    return {
      ...
      drinksFor,
      totalFor,
      addDrink: transaction(addDrink),
      undoToday: transaction(undoToday),
      deleteDrink: transaction(deleteDrink),
      resetDay: transaction(resetDay),
      restoreDay: transaction(restoreDay),
      saveCup: transaction(saveCup),
      deleteCup: transaction(deleteCup),
      saveSettings: transaction(saveSettings),
      replaceState: transaction(replaceState),
    };
  }
