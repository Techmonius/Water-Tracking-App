(function(){
  ...
  function track(event,data={}){const now=new Date();const payload={event,label:event.replaceAll('_',' '),summary:event.replaceAll('_',' '),data,installId:installId(),sessionId:sessionStorage.wtV1Session||(sessionStorage.wtV1Session=uid()),version:C.appVersion,device:device(),standalone:Boolean(matchMedia?.('(display-mode: standalone)').matches),localDate:now.toLocaleDateString(),localTime:now.toLocaleTimeString(),timestamp:now.toISOString()};try{fetch(ENDPOINT,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(payload),keepalive:true}).catch(()=>{});}catch{}}
