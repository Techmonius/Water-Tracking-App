(function(){
  ...
  function drawSprite(canvas,p,frame){
    ...
    if(p.stage>=1){
      const top=[16,13,11,8,6,4,3,2][p.stage];
      line(c,16,baseY,16,top,pal.stem,p.stage>=3?2:1);
    }
  ...
  window.addEventListener('storage',()=>render(false));window.addEventListener('wt-data-changed',()=>render(false));
